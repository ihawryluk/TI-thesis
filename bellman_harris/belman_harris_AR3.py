# -*- coding: utf-8 -*-
"""
Created on Sat Jul 18 11:26:42 2020
...
model {
  ...
  weekly_effect[4:(N+1)] ~ normal( weekly_effect[3:N]* weekly_rho + weekly_effect[2:(N-1)]* weekly_rho1 + weekly_effect[1:(N-2)]* weekly_rho2, weekly_sd);
  
  weekly_effect[2:3] ~ normal(-1,weekly_sd);
...
all_samples.to_csv('C:/Users/iwona/Desktop/TI/bellman_harris//models_fits/' + 'AR3' + '.csv', index=False)
...
# predictions_df['AR3'] = prediction_means
...
samples = all_samples[['phi', 'weekly_rho', 'weekly_rho1', 'weekly_rho2', 'weekly_sd', 'GI', 'R0']]
...
def Logf(theta_mean):
    ...
    weekly_sd = theta_mean[4]    
    GI = theta_mean[5]
    R0 = theta_mean[6]
    weekly_effect = theta_mean[7:].values
    ...
    we3_mu = weekly_effect[2:N] * weekly_rho + weekly_effect[1:(N-1)]* weekly_rho1 + weekly_effect[0:(N-2)]* weekly_rho2
    we3_sig = weekly_sd
    for i in range(3,N+1):
        log_prior += scipy.stats.norm(we3_mu[i-3], we3_sig).logpdf(weekly_effect[i])
...
laplaces = laplaces.append({'Logf': Logf(theta_mean), 
                            'LogLaplaceCovariance': LogLaplaceCovariance(theta_mean, 1.5*cov),
                            'GI': 'AR3_1.5_cov'}, ignore_index=True)
...
functions {
    ...
    real lf(vector theta, int N, int[] cases)
    {
     ...
     real weekly_sd = theta[5];
     real GI = theta[6];
     real R0 = theta[7];
     vector[N+1] weekly_effect = theta[8:(7+N+1)];
     
  
    // prepare the remaining parameters
    vector[N-2] we3_mu = weekly_effect[3:N]* weekly_rho + weekly_effect[2:(N-1)]* weekly_rho1 + weekly_effect[1:(N-2)]* weekly_rho2;
    real we3_sigma = weekly_sd;
   ...
   SI[1] = exp(-0.5*GI*1.5^2);  // is that needed? we're overwriting in the next line
    ...
    for (i in 2:N) {
     ...
     // priors
    for (i in 4:(N+1)){
     Lprior += normal_lpdf(weekly_effect[i] | we3_mu[i-3], we3_sigma);
    }
...
data {  
      ...
      int cases[N]; // reported deaths -- the rows with i > N contain -1 and should be ignored
      real x[N]; // x
      matrix[7 + N + 1,7 + N + 1] C;
      vector[7 + N + 1] peak; // mean parameters estimate
      real f0; // log(f(peak))
...
def get_expect_for_lambda(lam, data, n_iter=ITER):
    ...
    df.to_csv('' + 'GI_est_AR3_125_cov' + '_' + str(lam) + '.csv', index=False)
...
TI_dict_90_cov = get_TI_dict(1.25)
